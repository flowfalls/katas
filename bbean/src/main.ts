...
router.post("/roll", async (ctx) => {
  ...
  const gameController = new GameController(gameName as string, numberOfDice as number);
  const gameResult = returnMostCommonEntryInArray(gameController.startGame());

  ctx.response.status = 200;
  ctx.body = { result: `Thanks for playing ${gameController.gameName}, there are ${gameResult} of a kind!`};
});
